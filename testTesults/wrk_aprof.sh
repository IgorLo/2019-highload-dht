...
# Разогрев
wrk -s get.lua -c 4 -d 20 -t 4 -R 100 -L http://localhost:8080

#------ CPU

aprof start -e cpu -t -f output/1_get_empty_cpu.svg "$JPID"
wrk -s get.lua -c 4 -d $TIME -t 4 -R 100 -L http://localhost:8080 >> output/1_get_empty_wrk.txt
aprof stop -e cpu -t -f output/1_get_empty_cpu.svg "$JPID"

aprof start -e cpu -t -f output/2_put_cpu.svg "$JPID"
wrk -s put.lua -c 4 -d $TIME -t 4 -R 100 -L http://localhost:8080 >> output/2_put_wrk.txt
aprof stop -e cpu -t -f output/2_put_cpu.svg "$JPID"

aprof start -e cpu -t -f output/3_get_full_cpu.svg "$JPID"
wrk -s get.lua -c 4 -d $TIME -t 4 -R 100 -L http://localhost:8080 >> output/3_get_full_wrk.txt
aprof stop -e cpu -t -f output/3_get_full_cpu.svg "$JPID"

aprof start -e cpu -t -f output/4_delete_cpu.svg "$JPID"
wrk -s delete.lua -c 4 -d $TIME -t 4 -R 100 -L http://localhost:8080 >> output/4_delete_wrk.txt
aprof stop -e cpu -t -f output/4_delete_cpu.svg "$JPID"

aprof start -e cpu -t -f output/5_range_cpu.svg "$JPID"
wrk -s range.lua -c 4 -d $TIME -t 4 -R 100 -L http://localhost:8080 >> output/5_range_wrk.txt
